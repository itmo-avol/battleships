...
export class Game {
  rules = {
    ships: [4, 3, 3, 2, 2, 2, 1, 1, 1, 1], //количество кораблей
    field: {
      //размер поля
      width: 10,
      height: 10
  ...
  addShot(cell: Cell, player: Player, auto: boolean = false): Shot | undefined {
    ...
    let old_shot = this.shots.find(
      x => x.player == player && x.cell.x == cell.x && x.cell.y == cell.y
    );
    ...
    if (ship) {
      ship.hp -= 1;
      if (ship.hp <= 0) {
        let { startx, endx, starty, endy } = ship_ends(
          ship.length,
          ship.position
        );
...
function is_adj_ship(
  ...
  let {
    ...
    starty: starty1,
    endy: endy1
  } = ship_ends(ship_length1, position1);
  let {
    startx: startx2,
    endx: endx2,
    starty: starty2,
    endy: endy2
  } = ship_ends(ship_length2, position2);
...
function can_place(
  ...
  for (let sh of ships) {
    if (
      sh.position &&
      is_adj_ship(ship_length, pos, sh.length, sh.position, field)
    ) {
      return false;
    }
  }
...
function ship_fit_field(
  ...
  return (
    is_cell_valid(startx, endx, field) && is_cell_valid(starty, endy, field)
  );
}
