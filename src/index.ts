...
export function main() {
  ...
  renderer.backgroundColor = 0xffffff; // белый;
  stage = new PIXI.Container();

  PIXI.loader.add("assets/sprites.json").load(setup);
...
function animate_shot(sh_sprite: PIXI.Sprite) {
  ...
  sh_sprite.position.set(
    sh_sprite.position.x + sh_sprite.width / 2,
    sh_sprite.position.y + sh_sprite.height / 2
  );
